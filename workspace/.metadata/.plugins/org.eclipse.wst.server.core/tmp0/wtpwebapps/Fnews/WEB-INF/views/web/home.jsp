<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ include file="/common/taglib.jsp"%>

<html>
<head>
	<title>Trang chủ </title>

</head>

<body>
	<%-- <c:if test="${param.sessionTimeout != null}">
		<div class="alert alert-danger">
                   You haven't visited for too long
         </div>
	
	</c:if> --%>
<section class="bg0">
	<div class="container">
		<div class="row m-rl--1">
			<div class="col-sm-6 col-lg-4 p-rl-1 p-b-2">
				<div class="bg-img1 size-a-12 how1 pos-relative"
					style="background-image: url(/template/web/images/moinhat5.jpg);">
					<a href="#" class="dis-block how1-child1 trans-03"></a>

					<div class="flex-col-e-s s-full p-rl-25 p-tb-11">
						<a href="#"
							class="dis-block how1-child2 f1-s-2 cl0 bo-all-1 bocl0 hov-btn1 trans-03 p-rl-5 p-t-2">
							Thế giới
						</a>

						<h3 class="how1-child2 m-t-10">
							<a href="#" class="f1-m-1 cl0 hov-cl10 trans-03">
								Điều gì xảy ra nếu Ukraine kiểm soát nhà máy hạt nhân Kursk
							</a>
						</h3>
					</div>
				</div>
			</div>

			<div class="col-sm-6 col-lg-4 p-rl-1 p-b-2">
				<div class="bg-img1 size-a-12 how1 pos-relative"
					style="background-image: url(/template/web/images/moinhat6.jpg);">
					<a href="#" class="dis-block how1-child1 trans-03"></a>

					<div class="flex-col-e-s s-full p-rl-25 p-tb-11">
						<a href="#"
							class="dis-block how1-child2 f1-s-2 cl0 bo-all-1 bocl0 hov-btn1 trans-03 p-rl-5 p-t-2">
							Thế giới
						</a>

						<h3 class="how1-child2 m-t-10">
							<a href="#" class="f1-m-1 cl0 hov-cl10 trans-03">
								Chiến dịch Kursk tăng đà ủng hộ của phương Tây với Ukraine
							</a>
						</h3>
					</div>
				</div>
			</div>

			<div class="col-sm-6 col-lg-4 p-rl-1 p-b-2">
				<div class="bg-img1 size-a-12 how1 pos-relative"
					style="background-image: url(/template/web/images/moinhat7.jpg);">
					<a href="#" class="dis-block how1-child1 trans-03"></a>

					<div class="flex-col-e-s s-full p-rl-25 p-tb-11">
						<a href="#"
							class="dis-block how1-child2 f1-s-2 cl0 bo-all-1 bocl0 hov-btn1 trans-03 p-rl-5 p-t-2">
							Thời tiết
						</a>

						<h3 class="how1-child2 m-t-10">
							<a href="#" class="f1-m-1 cl0 hov-cl10 trans-03">
								Người Nhật bối rối với cảnh báo siêu động đất
							</a>
						</h3>
					</div>
				</div>
			</div>

			<div class="col-sm-6 col-lg-4 p-rl-1 p-b-2">
				<div class="bg-img1 size-a-12 how1 pos-relative"
					style="background-image: url(/template/web/images/khoahoc1.jpg);">
					<a href="#" class="dis-block how1-child1 trans-03"></a>

					<div class="flex-col-e-s s-full p-rl-25 p-tb-11">
						<a href="#"
							class="dis-block how1-child2 f1-s-2 cl0 bo-all-1 bocl0 hov-btn1 trans-03 p-rl-5 p-t-2">
							Khoa học
						</a>

						<h3 class="how1-child2 m-t-10">
							<a href="#" class="f1-m-1 cl0 hov-cl10 trans-03">
								AI4VN 2024 đề cập vấn đề nóng về AI tạo sinh
							</a>
						</h3>
					</div>
				</div>
			</div>

			<div class="col-sm-6 col-lg-4 p-rl-1 p-b-2">
				<div class="bg-img1 size-a-12 how1 pos-relative"
					style="background-image: url(/template/web/images/khoahoc2.jpg);">
					<a href="#" class="dis-block how1-child1 trans-03"></a>

					<div class="flex-col-e-s s-full p-rl-25 p-tb-11">
						<a href="#"
							class="dis-block how1-child2 f1-s-2 cl0 bo-all-1 bocl0 hov-btn1 trans-03 p-rl-5 p-t-2">
							Khoa học
						</a>

						<h3 class="how1-child2 m-t-10">
							<a href="#" class="f1-m-1 cl0 hov-cl10 trans-03">
								Australia chi 1,45 triệu AUD hỗ trợ đổi mới nông nghiệp bền vững tại Việt Nam
							</a>
						</h3>
					</div>
				</div>
			</div>

			<div class="col-sm-6 col-lg-4 p-rl-1 p-b-2">
				<div class="bg-img1 size-a-12 how1 pos-relative"
					style="background-image: url(/template/web/images/giaitri1.jpg);">
					<a href="#" class="dis-block how1-child1 trans-03"></a>

					<div class="flex-col-e-s s-full p-rl-25 p-tb-11">
						<a href="#"
							class="dis-block how1-child2 f1-s-2 cl0 bo-all-1 bocl0 hov-btn1 trans-03 p-rl-5 p-t-2">
							Giải trí
						</a>

						<h3 class="how1-child2 m-t-10">
							<a href="#" class="f1-m-1 cl0 hov-cl10 trans-03">
								
Ngọc Lan: 'Tôi ngừng đóng phim vì không còn thấy vui'
							</a>
						</h3>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>


<section class="bg0 p-t-50 p-b-90">
	<div class="container">
		<div class="row justify-content-center">
			<div class="col-md-10 col-lg-8 p-b-50">
				<div class="p-r-10 p-r-0-sr991">
					<div class="how2 how2-cl4 flex-s-c">
						<h3 class="f1-m-2 cl3 tab01-title">
							Tin tức mới nhất
						</h3>
					</div>

					<div class="p-b-40">
						<!-- Item post -->
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/moinhat1.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										Hàng trăm phụ huynh quây trường để xin học cho con
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by A
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Hàng trăm phụ huynh kéo đến trường Tiểu học Tây Mỗ 3 vì lo lắng con không được vào học, dù trường ở ngay đối diện khu chung cư.
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						<!-- Item post -->
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/moinhat2.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										AI4VN 2024 đề cập vấn đề nóng về AI tạo sinh
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by A
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Các chuyên gia, 'ông lớn công nghệ' sẽ phân tích xu hướng nổi bật, cơ hội bứt phá khi ứng dụng trí tuệ nhân tạo tạo sinh trong các lĩnh vực và quản trị doanh nghiệp tại AI4VN 2024.
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						<!-- Item post -->
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/moinhat3.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										Ukraine tập kích UAV quy mô lớn nhằm vào Moskva
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by A
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Giới chức Nga tuyên bố bắn hạ 10 UAV Ukraine, ngăn chặn một trong những đợt tập kích lớn nhất nhằm vào thủ đô Moskva.
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						<!-- Item post -->
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/moinhat4.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										Cổ phục trong phim kinh dị Tấm Cám
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by A
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Đoàn phim "Cám" đầu tư phục trang để tái hiện làng quê Việt thời phong kiến trong phim kinh dị lấy cảm hứng từ truyện cổ tích.
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						<!-- Item post -->
						

						<!-- Item post -->
						

						<!-- Item post -->
						
					</div>

					<a href="#" class="flex-c-c size-a-13 bo-all-1 bocl11 f1-m-6 cl6 hov-btn1 trans-03">
						Xem thêm
					</a>
				</div>
			</div>

			<div class="col-md-10 col-lg-4 p-b-50">
				<div class="p-l-10 p-rl-0-sr991">
					<!-- Banner -->

				</div>
			</div>
		</div>
	</div>
</section>

<!-- Post -->
<section class="bg0 p-t-70">
	<div class="container">
		<div class="row justify-content-center">
			<div class="col-md-10 col-lg-8">
				<div class="p-b-20">
					<!-- Tin tức  -->
					<div class="p-b-20">
						<div class="how2 how2-cl1 flex-sb-c m-r-10 m-r-0-sr991">
							<h3 class="f1-m-2 cl12 tab01-title">
								Thể thao 
							</h3>


							<a href="#" class="tab01-link f1-s-1 cl9 hov-cl10 trans-03">
								Xem thêm
								<i class="fs-12 m-l-5 fa fa-caret-right"></i>
							</a>
						</div>

						<div class="row p-t-35">
							<div class="col-sm-6 p-r-25 p-r-15-sr991">
								<!-- Item post -->
								<div class="m-b-30">
									<a href="#" class="wrap-pic-w hov1 trans-03">
										<img src="/template/web/images/bongda1.jpg" alt="IMG">
									</a>

									<div class="p-t-20">
										<h5 class="p-b-5">
											<a href="#" class="f1-m-3 cl2 hov-cl10 trans-03">
												
Nga gọi 38 cầu thủ chuẩn bị đấu Việt Nam
											</a>
										</h5>

										<span class="cl8">
											<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
												Thể thao 
											</a>

											<span class="f1-s-3 m-rl-3">
												-
											</span>

											<span class="f1-s-3">
												Feb 18
											</span>
										</span>
									</div>
								</div>
							</div>

							<div class="col-sm-6 p-r-25 p-r-15-sr991">
								<!-- Item post -->
								<div class="flex-wr-sb-s m-b-30">
									<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
										<img src="/template/web/images/bongda2.jpg"
											>
									</a>

									<div class="size-w-2">
										<h5 class="p-b-5">
											<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
												Hạ Nhật Bản, Việt Nam về nhì giải giao hữu U16
											</a>
										</h5>

										<span class="cl8">
											<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
												Thể thao 
											</a>

											<span class="f1-s-3 m-rl-3">
												-
											</span>

											<span class="f1-s-3">
												Feb 17
											</span>
										</span>
									</div>
									
								</div>

								<!-- Item post -->
								<div class="flex-wr-sb-s m-b-30">
									<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
										<img src="/template/web/images/bongda3.jpg"
											alt="IMGDonec metus orci, malesuada et lectus vitae">
									</a>

									<div class="size-w-2">
										<h5 class="p-b-5">
											<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
												Khánh Hòa dự giải, hạng Nhất giữ nguyên thể thức
											</a>
										</h5>

										<span class="cl8">
											<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
												Thể thao 
											</a>

											<span class="f1-s-3 m-rl-3">
												-
											</span>

											<span class="f1-s-3">
												Feb 16
											</span>
										</span>
									</div>
								</div>

								<!-- Item post -->
								<div class="flex-wr-sb-s m-b-30">
									<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
										<img src="/template/web/images/bongda4.png"
											alt="IMGDonec metus orci, malesuada et lectus vitae">
									</a>

									<div class="size-w-2">
										<h5 class="p-b-5">
											<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
												Thủ môn Đặng Văn Lâm có thể ra Hà Nội thi đấu
											</a>
										</h5>

										<span class="cl8">
											<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
												Thể thao 
											</a>

											<span class="f1-s-3 m-rl-3">
												-
											</span>

											<span class="f1-s-3">
												Feb 12
											</span>
										</span>
									</div>
								</div>
							</div>
						</div>
					</div>

					<!-- Other -->
					<div class="row">
						<!-- Business -->
						<div class="col-sm-6 p-r-25 p-r-15-sr991 p-b-25">
							<div class="how2 how2-cl2 flex-sb-c m-b-35">
								<h3 class="f1-m-2 cl13 tab01-title">
									Chính trị
								</h3>


								<a href="#" class="tab01-link f1-s-1 cl9 hov-cl10 trans-03">
									Xem thêm
									<i class="fs-12 m-l-5 fa fa-caret-right"></i>
								</a>
							</div>

							<!-- Main Item post -->
							<div class="m-b-30">
								<a href="#" class="wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/dansinh1.jpg" alt="IMG">
								</a>

								<div class="p-t-20">
									<h5 class="p-b-5">
										<a href="#" class="f1-m-3 cl2 hov-cl10 trans-03">
											Kinh phí đền bù ở dự án kênh Đôi tăng lên 6.000 tỷ đồng
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
											Chính trị
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 18
										</span>
									</span>
								</div>
							</div>

							<!-- Item post -->
							<div class="flex-wr-sb-s m-b-30">
								<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/dansinh2.jpg"
										alt="IMGDonec metus orci, malesuada et lectus vitae">
								</a>

								<div class="size-w-2">
									<h5 class="p-b-5">
										<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
											Cô giáo cưu mang nhiều học trò mồ côi
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
											Chính trị
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 17
										</span>
									</span>
								</div>
							</div>

							<!-- Item post -->
							<div class="flex-wr-sb-s m-b-30">
								<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/dansinh3.jpg"
										alt="IMGDonec metus orci, malesuada et lectus vitae">
								</a>

								<div class="size-w-2">
									<h5 class="p-b-5">
										<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
											Trồng ớt A Riêu đặc sản
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
											Chính trị
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 16
										</span>
									</span>
								</div>
							</div>
						</div>



						<!-- Thời sụ -->
						<div class="col-sm-6 p-r-25 p-r-15-sr991 p-b-25">
							<div class="how2 how2-cl6 flex-sb-c m-b-35">
								<h3 class="f1-m-2 cl18 tab01-title">
									Thời sự
								</h3>


								<a href="#" class="tab01-link f1-s-1 cl9 hov-cl10 trans-03">
									Xem thêm
									<i class="fs-12 m-l-5 fa fa-caret-right"></i>
								</a>
							</div>

							<!-- Main Item post -->
							<div class="m-b-30">
								<a href="blog-detail-01.html" class="wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/thoisu1.jpg" alt="IMG">
								</a>

								<div class="p-t-20">
									<h5 class="p-b-5">
										<a href="#" class="f1-m-3 cl2 hov-cl10 trans-03">
											Ba bộ trưởng trả lời chất vấn sáng nay
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
											Thời sự
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 18
										</span>
									</span>
								</div>
							</div>

							<!-- Item post -->
							<div class="flex-wr-sb-s m-b-30">
								<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/thoisu2.jpg"
										alt="IMGDonec metus orci, malesuada et lectus vitae">
								</a>

								<div class="size-w-2">
									<h5 class="p-b-5">
										<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
											'Trả lương phù hợp với cống hiến để không cần tham nhũng'
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
											Thời sự
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 17
										</span>
									</span>
								</div>
							</div>

							<!-- Item post -->
							<div class="flex-wr-sb-s m-b-30">
								<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/thoisu3.jpg"
										alt="IMGDonec metus orci, malesuada et lectus vitae">
								</a>

								<div class="size-w-2">
									<h5 class="p-b-5">
										<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
											Việt - Trung ra tuyên bố chung
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
											Thời sự
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 16
										</span>
									</span>
								</div>
							</div>
						</div>

						<!-- Thế giới -->
						<div class="col-sm-6 p-r-25 p-r-15-sr991 p-b-25">
							<div class="how2 how2-cl5 flex-sb-c m-b-35">
								<h3 class="f1-m-2 cl17 tab01-title">
									Thế giới
								</h3>


								<a href="#" class="tab01-link f1-s-1 cl9 hov-cl10 trans-03">
									View all
									<i class="fs-12 m-l-5 fa fa-caret-right"></i>
								</a>
							</div>

							<!-- Main Item post -->
							<div class="m-b-30">
								<a href="#" class="wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/tulieu1.jpg" alt="IMG">
								</a>

								<div class="p-t-20">
									<h5 class="p-b-5">
										<a href="#" class="f1-m-3 cl2 hov-cl10 trans-03">
											Người ủng hộ lo lắng với chiến thuật tranh cử của Trump
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
											Thế giới
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 18
										</span>
									</span>
								</div>
							</div>

							<!-- Item post -->
							<div class="flex-wr-sb-s m-b-30">
								<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/tulieu2.jpg"
										alt="IMGDonec metus orci, malesuada et lectus vitae">
								</a>

								<div class="size-w-2">
									<h5 class="p-b-5">
										<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
											Binh nghiệp gây tranh cãi của phó tướng bà Harris
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
											Thế giới
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 17
										</span>
									</span>
								</div>
							</div>

							<!-- Item post -->
							<div class="flex-wr-sb-s m-b-30">
								<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/tulieu3.jpg"
										alt="IMGDonec metus orci, malesuada et lectus vitae">
								</a>

								<div class="size-w-2">
									<h5 class="p-b-5">
										<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
											Chính sách có thể giúp tân Thủ tướng Thái Lan vực dậy kinh tế
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
											Thế giới
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 16
										</span>
									</span>
								</div>
							</div>
						</div>

						<!-- Giải trí -->
						<div class="col-sm-6 p-r-25 p-r-15-sr991 p-b-25">
							<div class="how2 how2-cl6 flex-sb-c m-b-35">
								<h3 class="f1-m-2 cl18 tab01-title">
									Giải trí
								</h3>


								<a href="#" class="tab01-link f1-s-1 cl9 hov-cl10 trans-03">
									Xem thêm
									<i class="fs-12 m-l-5 fa fa-caret-right"></i>
								</a>
							</div>

							<!-- Main Item post -->
							<div class="m-b-30">
								<a href="#" class="wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/giaitri1.jpg" alt="IMG">
								</a>

								<div class="p-t-20">
									<h5 class="p-b-5">
										<a href="#" class="f1-m-3 cl2 hov-cl10 trans-03">
											Ngọc Lan: 'Tôi ngừng đóng phim vì không còn thấy vui'
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
											Giải trí
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 18
										</span>
									</span>
								</div>
							</div>

							<!-- Item post -->
							<div class="flex-wr-sb-s m-b-30">
								<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/giaitri2.jpg"
										alt="IMGDonec metus orci, malesuada et lectus vitae">
								</a>

								<div class="size-w-2">
									<h5 class="p-b-5">
										<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
											Bộ sưu tập Labubu của hoa hậu Kỳ Duyên
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
											Giải trí
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 17
										</span>
									</span>
								</div>
							</div>

							<!-- Item post -->
							<div class="flex-wr-sb-s m-b-30">
								<a href="#" class="size-w-1 wrap-pic-w hov1 trans-03">
									<img src="/template/web/images/giaitri3.jpg"
										alt="IMGDonec metus orci, malesuada et lectus vitae">
								</a>

								<div class="size-w-2">
									<h5 class="p-b-5">
										<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
											Taylor Swift gặp fan nhí sống sót sau vụ đâm dao ở Anh
										</a>
									</h5>

									<span class="cl8">
										<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
											Giải trí
										</a>

										<span class="f1-s-3 m-rl-3">
											-
										</span>

										<span class="f1-s-3">
											Feb 16
										</span>
									</span>
								</div>
							</div>
						</div>
					</div>

					<!-- Travel  -->
					<!-- <div class="p-b-25 p-r-10 p-r-0-sr991">
						<div class="how2 how2-cl3 flex-s-c">
							<h3 class="f1-m-2 cl14 tab01-title">
								Travel
							</h3>
						</div>

						<div class="flex-wr-sb-s p-t-35">
							<div class="size-w-6 w-full-sr575">
								Item post
								<div class="m-b-30">
									<a href="#" class="wrap-pic-w hov1 trans-03">
										<img src="/template/web/images/post-14.jpg" alt="IMG">
									</a>

									<div class="p-t-25">
										<h5 class="p-b-5">
											<a href="#" class="f1-m-3 cl2 hov-cl10 trans-03">
												You wish lorem ipsum dolor sit amet consectetur
											</a>
										</h5>

										<span class="cl8">
											<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
												Beach
											</a>

											<span class="f1-s-3 m-rl-3">
												-
											</span>

											<span class="f1-s-3">
												Feb 18
											</span>
										</span>

										<p class="f1-s-1 cl6 p-t-18">
											Duis eu felis id tortor congue consequat. Sed vitae vestibulum enim, et
											pharetra magna
										</p>
									</div>
								</div>
							</div>

							<div class="size-w-7 w-full-sr575">
								Item post
								<div class="m-b-30">
									<a href="#" class="wrap-pic-w hov1 trans-03">
										<img src="/template/web/images/post-15.jpg" alt="IMG">
									</a>

									<div class="p-t-10">
										<h5 class="p-b-5">
											<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
												Donec metus orci, malesuada et lectus vitae
											</a>
										</h5>

										<span class="cl8">
											<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
												Beach
											</a>

											<span class="f1-s-3 m-rl-3">
												-
											</span>

											<span class="f1-s-3">
												Feb 17
											</span>
										</span>
									</div>
								</div>

								Item post
								<div class="m-b-30">
									<a href="#" class="wrap-pic-w hov1 trans-03">
										<img src="/template/web/images/post-17.jpg" alt="IMG">
									</a>

									<div class="p-t-10">
										<h5 class="p-b-5">
											<a href="#" class="f1-s-5 cl3 hov-cl10 trans-03">
												Donec metus orci, malesuada et lectus vitae
											</a>
										</h5>

										<span class="cl8">
											<a href="#" class="f1-s-6 cl8 hov-cl10 trans-03">
												Hotels
											</a>

											<span class="f1-s-3 m-rl-3">
												-
											</span>

											<span class="f1-s-3">
												Feb 16
											</span>
										</span>
									</div>
								</div>
							</div>
						</div>
					</div> -->
				</div>
			</div>

			<div class="col-md-10 col-lg-4">
				<div class="p-l-10 p-rl-0-sr991 p-b-20">
					<!-- Stay Connected -->


					<!-- Phổ biến -->
					<div class="p-b-30">
						<div class="how2 how2-cl4 flex-s-c">
							<h3 class="f1-m-2 cl3 tab01-title">
								Phổ biến
							</h3>
						</div>

						<ul class="p-t-35">
							<li class="flex-wr-sb-s p-b-22">
								<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0 m-b-6">
									1
								</div>

								<a href="#" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">
									Những tinh chất điện di làm đẹp phổ biến
								</a>
							</li>

							<li class="flex-wr-sb-s p-b-22">
								<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0 m-b-6">
									2
								</div>

								<a href="#" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">
									Triệu chứng của 9 ung thư phổ biến
								</a>
							</li>

							<li class="flex-wr-sb-s p-b-22">
								<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0 m-b-6">
									3
								</div>

								<a href="#" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">
									Sỏi tiết niệu và 4 nguyên nhân phổ biến
								</a>
							</li>

							<li class="flex-wr-sb-s p-b-22">
								<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0 m-b-6">
									4
								</div>

								<a href="#" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">
									Tại sao nguyên tố phổ biến nhất vũ trụ quá hiếm trên Trái Đất

								</a>
							</li>

							<li class="flex-wr-sb-s p-b-22">
								<div class="size-a-8 flex-c-c borad-3 size-a-8 bg9 f1-m-4 cl0">
									5
								</div>

								<a href="#" class="size-w-3 f1-s-7 cl3 hov-cl10 trans-03">
									6 hiểu lầm phổ biến về vaccine dại
								</a>
							</li>
						</ul>
					</div>

					<!--  -->


					<!-- Video -->
				<!-- 	<div class="p-b-55">
						<div class="how2 how2-cl4 flex-s-c m-b-35">
							<h3 class="f1-m-2 cl3 tab01-title">
								Featured Video
							</h3>
						</div>

						<div>
							<div class="wrap-pic-w pos-relative">
								<img src="/template/web/images/video-01.jpg" alt="IMG">

								<button class="s-full ab-t-l flex-c-c fs-32 cl0 hov-cl10 trans-03" data-toggle="modal"
									data-target="#modal-video-01">
									<span class="fab fa-youtube"></span>
								</button>
							</div>

							<div class="p-tb-16 p-rl-25 bg3">
								<h5 class="p-b-5">
									<a href="#" class="f1-m-3 cl0 hov-cl10 trans-03">
										Music lorem ipsum dolor sit amet consectetur
									</a>
								</h5>

								<span class="cl15">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by John Alvarado
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</span>
							</div>
						</div>
					</div> -->

					<!-- Subscribe -->
					<!-- <div class="bg10 p-rl-35 p-t-28 p-b-35 m-b-55">
						<h5 class="f1-m-5 cl0 p-b-10">
							Subscribe
						</h5>

						<p class="f1-s-1 cl0 p-b-25">
							Get all latest content delivered to your email a few times a month.
						</p>

						<form class="size-a-9 pos-relative">
							<input class="s-full f1-m-6 cl6 plh9 p-l-20 p-r-55" type="text" name="email"
								placeholder="Email">

							<button class="size-a-10 flex-c-c ab-t-r fs-16 cl9 hov-cl10 trans-03">
								<i class="fa fa-arrow-right"></i>
							</button>
						</form>
					</div> -->

					<!-- Tag -->
					<div class="p-b-55">
						<div class="how2 how2-cl4 flex-s-c m-b-30">
							<h3 class="f1-m-2 cl3 tab01-title">
								Tags
							</h3>
						</div>

						<div class="flex-wr-s-s m-rl--5">
							<a href="#"
								class="flex-c-c size-h-2 bo-1-rad-20 bocl12 f1-s-1 cl8 hov-btn2 trans-03 p-rl-20 p-tb-5 m-all-5">
								Thời trang
							</a>

							<a href="#"
								class="flex-c-c size-h-2 bo-1-rad-20 bocl12 f1-s-1 cl8 hov-btn2 trans-03 p-rl-20 p-tb-5 m-all-5">
								Công nghệ
							</a>

							<a href="#"
								class="flex-c-c size-h-2 bo-1-rad-20 bocl12 f1-s-1 cl8 hov-btn2 trans-03 p-rl-20 p-tb-5 m-all-5">
								Kinh doanh
							</a>

							<a href="#"
								class="flex-c-c size-h-2 bo-1-rad-20 bocl12 f1-s-1 cl8 hov-btn2 trans-03 p-rl-20 p-tb-5 m-all-5">
								Du lịch
							</a>

							<a href="#"
								class="flex-c-c size-h-2 bo-1-rad-20 bocl12 f1-s-1 cl8 hov-btn2 trans-03 p-rl-20 p-tb-5 m-all-5">
								Đời sống
							</a>

							<a href="#"
								class="flex-c-c size-h-2 bo-1-rad-20 bocl12 f1-s-1 cl8 hov-btn2 trans-03 p-rl-20 p-tb-5 m-all-5">
								Giáo dục
							</a>

							<a href="#"
								class="flex-c-c size-h-2 bo-1-rad-20 bocl12 f1-s-1 cl8 hov-btn2 trans-03 p-rl-20 p-tb-5 m-all-5">
								Pháp luật
							</a>

							<a href="#"
								class="flex-c-c size-h-2 bo-1-rad-20 bocl12 f1-s-1 cl8 hov-btn2 trans-03 p-rl-20 p-tb-5 m-all-5">
								Nhịp sống
							</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>

<!-- Banner -->


<!-- Latest -->
<!-- <section class="bg0 p-t-50 p-b-90">
	<div class="container">
		<div class="row justify-content-center">
			<div class="col-md-10 col-lg-8 p-b-50">
				<div class="p-r-10 p-r-0-sr991">
					<div class="how2 how2-cl4 flex-s-c">
						<h3 class="f1-m-2 cl3 tab01-title">
							Latest Articles
						</h3>
					</div>

					<div class="p-b-40">
						Item post
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/post-43.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										Robot lorem ipsum dolor sit amet consectetur
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by John Alvarado
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Duis eu felis id tortor congue consequat. Sed vitae vestibulum enim, et pharetra
									magna
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						Item post
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/post-44.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										Health lorem ipsum dolor sit amet consectetur
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by John Alvarado
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Duis eu felis id tortor congue consequat. Sed vitae vestibulum enim, et pharetra
									magna
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						Item post
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/post-45.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										Success lorem ipsum dolor sit amet consectetur
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by John Alvarado
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Duis eu felis id tortor congue consequat. Sed vitae vestibulum enim, et pharetra
									magna
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						Item post
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/post-46.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										Bitcon lorem ipsum dolor sit amet consectetur
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by John Alvarado
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Duis eu felis id tortor congue consequat. Sed vitae vestibulum enim, et pharetra
									magna
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						Item post
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/post-47.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										American Bank lorem ipsum dolor sit amet
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by John Alvarado
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Duis eu felis id tortor congue consequat. Sed vitae vestibulum enim, et pharetra
									magna
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						Item post
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/post-48.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										Macbook New 12 lorem ipsum dolor sit amet
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by John Alvarado
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Duis eu felis id tortor congue consequat. Sed vitae vestibulum enim, et pharetra
									magna
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>

						Item post
						<div class="flex-wr-sb-s p-t-40 p-b-15 how-bor2">
							<a href="#" class="size-w-8 wrap-pic-w hov1 trans-03 w-full-sr575 m-b-25">
								<img src="/template/web/images/post-49.jpg" alt="IMG">
							</a>

							<div class="size-w-9 w-full-sr575 m-b-25">
								<h5 class="p-b-12">
									<a href="#" class="f1-l-1 cl2 hov-cl10 trans-03 respon2">
										You wish lorem ipsum dolor sit amet consectetur
									</a>
								</h5>

								<div class="cl8 p-b-18">
									<a href="#" class="f1-s-4 cl8 hov-cl10 trans-03">
										by John Alvarado
									</a>

									<span class="f1-s-3 m-rl-3">
										-
									</span>

									<span class="f1-s-3">
										Feb 18
									</span>
								</div>

								<p class="f1-s-1 cl6 p-b-24">
									Duis eu felis id tortor congue consequat. Sed vitae vestibulum enim, et pharetra
									magna
								</p>

								<a href="#" class="f1-s-1 cl9 hov-cl10 trans-03">
									Read More
									<i class="m-l-2 fa fa-long-arrow-alt-right"></i>
								</a>
							</div>
						</div>
					</div>

					<a href="#" class="flex-c-c size-a-13 bo-all-1 bocl11 f1-m-6 cl6 hov-btn1 trans-03">
						Load More
					</a>
				</div>
			</div>

			<div class="col-md-10 col-lg-4 p-b-50">
				<div class="p-l-10 p-rl-0-sr991">
					Banner

				</div>
			</div>
		</div>
	</div>
</section> -->

</body>

</html>







